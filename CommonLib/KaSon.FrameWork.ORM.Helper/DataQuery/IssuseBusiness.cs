...
namespace KaSon.FrameWork.ORM.Helper
{
    public class IssuseBusiness:DBbase
    {
        ...
        public void UpdateBonusPool_SZC(string gameCode, string issuseNumber)
        {
            ...
            using (bonusManager.DB)
            {
                bonusManager.DB.Begin();
                ...
                try
                {
                    ...
                    bonusManager.DB.Commit();
                }
                catch (Exception)
                {
                    bonusManager.DB.Rollback();
                    throw;
                }
            }
        ...
        //    return resultList;
            var coll = mDB.GetCollection<SZC_BonusPoolInfo>("SZC_BonusPoolInfo");
            var builder = Builders<SZC_BonusPoolInfo>.Filter;
            ...
            var filter = builder.Eq("IssuseNumber", issuseNumber) & builder.Eq("GameCode", gameCode);// & builder.Lte("IssuseNumber", issuseNumber);
            var documents = coll.Find<SZC_BonusPoolInfo>(filter).FirstOrDefault();
