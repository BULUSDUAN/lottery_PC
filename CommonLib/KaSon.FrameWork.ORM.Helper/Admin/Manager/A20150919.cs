...
namespace KaSon.FrameWork.ORM.Helper
{
    ...
    /// 
    /// </summary>
    public class Yqid_Regist_GiveRedBag : DBbase, IyqidRegister_AfterTranCommit
    {
    ...
    /// 注册就送5元红包
    /// </summary>
    public class A20150919_Regist_GiveRedBag : DBbase, IRegister_AfterTranCommit
    {
    ...
    /// </summary>
    public class A20150919_BindRealName_Mobile_Login : DBbase, IUser_AfterLogin
    {
    ...
    public class A20150919_FillMoneyGive : DBbase, ICompleteFillMoney_AfterTranCommit
    {
        ...
        public void UpdateFillMoneyGiveRedBagConfig(int id, decimal fillMoney, decimal giveMoney)
        {
            ...
            if (old == null)
                throw new LogicException("找不到相应的配置");
    ...
    public class A20150919_AddBonusMoney :DBbase, IOrderPrize_AfterTranCommit
    {
        ...
        public void ManualAddMoney(string schemeId)
        {
            ...
            if (order == null)
                throw new LogicException(string.Format("加奖，没有找到订单{0}", schemeId));
            if (order.AddMoney > 0)
                BusinessHelper.Payin_To_Balance(order.AddMoneyDescription == "10" ? AccountType.Bonus : AccountType.RedBag, BusinessHelper.FundCategory_Bonus, order.UserId, schemeId, order.AddMoney,string.Format("订单{0}活动赠送{1:N2}元.", schemeId, order.AddMoney));
            return;
        }
