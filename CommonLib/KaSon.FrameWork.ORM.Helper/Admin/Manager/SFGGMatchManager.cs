   ...
   public class SFGGMatchManager:DBbase
    {
        ...
        {
            DB.GetDal<C_SFGG_Match>().Add(entity);
        }
        public void UpdateSFGGMatch(C_SFGG_Match entity)
        {
            DB.GetDal<C_SFGG_Match>().Update(entity);
        }
        public List<C_SFGG_Match> QuerySFGGMatchList(string issuseNumber)
        {
            return DB.CreateQuery<C_SFGG_Match>().Where(s => s.IssuseNumber == issuseNumber).ToList();
        }
        public C_SFGG_Match QuerySFGGMatch(string issuseNumber, int matchOrderId)
        {
            return DB.CreateQuery<C_SFGG_Match>().Where(s => s.IssuseNumber == issuseNumber && s.MatchOrderId == matchOrderId).FirstOrDefault();
        }
    }
}
