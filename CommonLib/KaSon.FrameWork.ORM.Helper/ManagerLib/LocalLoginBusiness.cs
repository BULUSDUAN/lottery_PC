...
namespace KaSon.FrameWork.ORM.Helper
{
    public class LocalLoginBusiness : DBbase
    {
        ...
        {
            return new LoginLocalManager().QueryNotOnlineRecentlyList(days, pageIndex, pageSize);
        }
        public string GiveMoneyToStayUser(string userId, string operatorId)
        {
            //查询属于哪个档次的
            //送现金及红包
           return  new LoginLocalManager().GiveMoneyToStayUser(userId, operatorId);
        }
        public NotOnlineUserCollection QueryNotOnlineRecentlyList(int days, int pageIndex, int pageSize, string theEarnings)
        {
           return new LoginLocalManager().QueryNotOnlineRecentlyList(days, pageIndex, pageSize, theEarnings);
        }
        /// <summary>
        /// 查找最近未登录用户列表(导出)
        /// </summary>
        /// <param name="days"></param>
        /// <param name="theEarnings"></param>
        /// <returns></returns>
        public NotOnlineUserCollection QueryExcelNotOnlineRecentlyList(int days, string theEarnings)
        {
            return new LoginLocalManager().QueryExcelNotOnlineRecentlyList(days, theEarnings);
        }
        public UserQueryInfo QueryUserByKey(string userId, string agentId)
        {
            ...
            return new UserQueryInfo
            {
                ...
                CPSMode = ocAgent == null ? 0 : (CPSMode)ocAgent.CPSMode,
                UserGrowth = balance.UserGrowth,
                CPSBalance = balance.CPSBalance,
                UserCreditType = reg.UserCreditType
            };
        }
