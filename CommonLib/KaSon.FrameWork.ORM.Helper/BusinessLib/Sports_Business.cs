    ...
    public class Sports_Business: DBbase
    {
       ...
       public TicketCollection AnalyzeOrder(GatewayTicketOrder order)
            ...
            if (oldCount <= 0)
            {
                ...
                else
                {
                    ...
                    if (jcGameCodeArray.Contains(order.GameCode))
                    {
                        ...
                        {
                            //生成文件
                            var json = Common.JsonHelper.Serialize(betInfo);
                            var path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "CacheData", "Orders", DateTime.Today.ToString("yyyyMMdd"), order.GameCode, schemeId.Substring(0, 10));
                            if (!Directory.Exists(path))
                                Directory.CreateDirectory(path);
                            var fileName = Path.Combine(path, string.Format("{0}.json", schemeId));
        ...
        private void SerializChaseOrder(LotteryBettingInfo order, string chaseOrderId)
        {
            try
            {
                var json = Common.JsonHelper.Serialize(order);
        ...
        public void BettingUserSavedOrder(string schemeId, string userId, string balancePassword, decimal redBagMoney)
        {
            ...
            if (saveOrder.ProgressStatus != (int)ProgressStatus.Waitting)
            ...
            try
            {
                ...
                order.SchemeType = isHM ? (int)SchemeType.TogetherBetting : (int)SchemeType.GeneralBetting;
                order.RedBagMoney = redBagMoney;
                order.TicketLog += "|" + DateTime.Now + "用户购买保存订单|";
                sportsManager.UpdateSports_Order_Running(order);

                orderDetail.SchemeType = isHM ? (int)SchemeType.TogetherBetting : (int)SchemeType.GeneralBetting;
                ...
                if (!isHM)
                {
                    ...
                    string msg = noIssuseGameCode.Contains(order.GameCode) ? string.Format("{0} 投注", BettingHelper.FormatGameCode(order.GameCode))
                                                        : string.Format("{0}第{1}期投注", BettingHelper.FormatGameCode(order.GameCode), order.IssuseNumber);
                    if (redBagMoney > 0M)
                    {
                        //红包支付
                        BusinessHelper.Payout_RedBag_To_End(BusinessHelper.FundCategory_Betting, userId, schemeId, redBagMoney, msg, "Bet", balancePassword);
        ...
        public void BettingSavedCreateTogether(string schemeId, string balancePassword, out bool canChase, out bool isHMRenGou)
        {
            ...
            try
            {
                ...
                if (joinTogetherList != null && joinTogetherList.Count > 0)
                {
                    ...
                    {
                        if (item.JoinType == (int)TogetherJoinType.Subscription)
                        {
                        ...
                        else if (item.JoinType == (int)TogetherJoinType.FollowerJoin)//自动跟单扣款
                        {
                            try
                            {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                Log4Log.Error("跟单合买扣款异常-BettingSavedCreateTogether"+"跟单合买扣款异常-BettingSavedCreateTogether"+strMsg, ex);
                            }
                        }
                        else if (item.JoinType == (int)TogetherJoinType.Guarantees)
                        {
        ...
        private void CheckPrivilegesType_JCZQ_Single(string gameCode, string gameType, List<string> jcCodeList, List<C_JCZQ_Match> jczqDsMatchList)
        {
            //PrivilegesType
            ...
            //开启事务
                ...
                if (redBagMoney > 0M)
                {
                    ...
                    if (redBagMoney > maxUseMoney)
                        throw new Exception(string.Format("本彩种只允许使用红包为订单总金额的{0}%，即{1:N2}元", percent, maxUseMoney));
                    //红包支付
                    BusinessHelper.Payout_RedBag_To_End(BusinessHelper.FundCategory_Betting, userId, schemeId, redBagMoney, string.Format("{0}单式上传消费{1:N2}元", BettingHelper.FormatGameCode(info.GameCode), redBagMoney), "Bet", balancePassword);
                }
                // 消费资金
                BusinessHelper.Payout_To_End(BusinessHelper.FundCategory_Betting, userId, schemeId, info.TotalMoney - redBagMoney,
                     string.Format("{0}单式上传消费{1:N2}元", BettingHelper.FormatGameCode(info.GameCode), info.TotalMoney - redBagMoney), "Bet", balancePassword);
        ...
        /// 拆票（无线程）
        /// </summary>
        public string DoSplitOrderTicketsWithNoThread(string schemeId, C_Sports_Order_Running order)
        {
            var log = new List<string>();
            try
            {
                ...
                if (oldCount <= 0)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            RequestTicket2(betInfo, "", true, (SchemeType)order.SchemeType);
                            //new Thread(() =>
                            //{
        ...
        public string DoOrderPrize(string schemeId)
        {
            ...
            try
            {
                var ticketList = manager.QueryTicketList(schemeId).ToList();
                if (ticketList.Count <= 0)
                    return string.Format("订单{0}没有生成票数据", schemeId);
                //订单的票中包括未派奖的票
                if (ticketList.Where(p => p.BonusStatus != (int)BonusStatus.Lose && p.BonusStatus != (int)BonusStatus.Win).Count() > 0)
