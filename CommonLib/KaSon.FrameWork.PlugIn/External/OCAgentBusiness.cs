...
namespace KaSon.FrameWork.PlugIn.External
{
    ...
    /// 代理相关业务
    /// </summary>
    public class OCAgentBusiness :DBbase, IPayBackTicket, IAgentPayIn
    {
        ...
        //退还用户返点
        private void PayBackRebate(OCAgentManager manager, C_User_Register currentUser, string schemeId, string orderUserId, SchemeType schemeType, string gameCode, string gameType, decimal totalOrderMoney, decimal payBackMoney, decimal payedRebate)
        {
            if (currentUser == null)
                return;
        ...
        /// 查询代理类型
        /// </summary>
        public P_OCAgent QueryAgentType(string userid)
        {
            var agentManager = new OCAgentManager();
            var userAgent = agentManager.QueryOCAgent(userid);
            return userAgent;
        ...
        public SubUserNoPayRebateOrderInfoCollection QuerySubUserCreateingOrderList(string key, string userId, DateTime starTime, DateTime endTime, int pageIndex, int pageSize)
        {
            ...
            if (!string.IsNullOrEmpty(key))
            {
                ...
                var user = manager.LoadUserRegister(key);
        ...
        public void UpdateUserAgentId(string userId, string agentId)
        {
            ...
            try
            {
                ...
                var reg = manager.LoadUserRegister(userId);
                ...
                var parentAgent = manager.LoadUserRegister(agentId);
        ...
        public void UpdateRelationChildRebate(string currUserId, string oldAgentId, string newAgentId, string path)
        {
            ...
            foreach (var item in xjUser)
            {
                ...
                if (!strUserIds.Contains(item.UserId))
                {
                    ...
                    var userRegist = userManager.LoadUserRegister(item.UserId);
                    if (userRegist != null)
                    {
