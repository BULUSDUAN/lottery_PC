...
{
    internal class GameAnalyzer_数字彩 : IOrderAnalyzable, IWinNumberAnalyzable
    {
        public GameAnalyzer_数字彩(string gameCode)
        {
            GameCode = gameCode;
            Spliter_Level1 = '|';
            Spliter_Level2 = ',';
        ...
        public bool CheckOneAntecodeNumber_First(string antecodeNumber, out string errMsg)
        {
            ...
            if (UInt16.TryParse(antecodeNumber, out number))
            {
                ...
                {
                    errMsg = string.Format("前区号码必须是 {0,1:D1} - {1,1:D1} 之间的数字", MinNumber_First, MaxNumber_First);
                    return false;
                }
            }
            else
