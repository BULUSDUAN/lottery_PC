using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace KaSon.FrameWork.ORM.Helper.WinNumber.Model
{
    /// <summary>
    /// 和值走势
    /// </summary>
    public class DF6_1_HZZS : ImportBase
    {

        public virtual int Red_06 { get; set; }
        public virtual int Red_712 { get; set; }
        public virtual int Red_1318 { get; set; }
        public virtual int Red_1924 { get; set; }
        public virtual int Red_2530 { get; set; }
        public virtual int Red_3136 { get; set; }
        public virtual int Red_3742 { get; set; }
        public virtual int Red_4348 { get; set; }
        public virtual int Red_4954 { get; set; }

        public virtual int HeZhi { get; set; }

        public virtual int JO_J { get; set; }
        public virtual int JO_O { get; set; }
        public virtual int DX_D { get; set; }
        public virtual int DX_X { get; set; }
        public virtual int ZH_Z { get; set; }
        public virtual int ZH_H { get; set; }


        public virtual int C3_0 { get; set; }
        public virtual int C3_1 { get; set; }
        public virtual int C3_2 { get; set; }
    }
}
