...
namespace KaSon.FrameWork.Common.Shunlifu
{
    public class SLF_PAY
    {
        ...
        public string ChargeAlipay(ushort type, ushort channel, string order, decimal amount, string ip, string Ua, string subject, string body, string remark)
        {
            try
            {
                var request = new ChargeAlipayNative()
                   {
                       Type = type,
                       Channel = channel,
                       Order = order,
                       Currency = 1,
                       Amount = amount,
                       Client = new ChargeClient()
                       {
                           Ip = ip,
                           Ua = Ua
                       },
                       Product = new ChargeProduct()
                       {
                           Subject = subject,
                           Body = body,
                       },
                       Time = DateTimeHelper.LocalDateTimeToUnixTimeStamp(DateTime.Now),
                       Timeout = 60 * 10,
                       Remark = remark
                   };

                ChargeAlipayNativeResult response;
                var result = ChinaRailwayApp.GetInstance(AppId, PrivateKey, gateway_url).Charge.ChargeAlipayNative(request, out response); // 调用接口生成订单
        ...
        public string ChargeWeixin(ushort type, ushort channel, string order, decimal amount, string ip, string Ua, string subject, string body, string remark)
        {
            try
            {
                var request = new ChargeWepayNative()
                {
                    Type = type,
                    Channel = channel,
                    Order = order,
        ...
        public string ChargeQQ(ushort type, ushort channel, string order, decimal amount, string ip, string Ua, string subject, string body, string remark)
        {
            try
            {
                var request = new ChargeQQpayNative()
                {
                    Type = type,
                    Channel = channel,
                    Order = order,
        ...
        public string Chargeexpress(ushort type, ushort channel, string order, decimal amount, string ip, string Ua, string subject, string body, string remark, string resultUrl, string no, string name, string cert_id, int cert_type, string mobile)
        {
            try
            {
                var request = new ChargeBankEasy(channel, BankAccountType.DebitCard, no)
                {
                    //Type = type,
                    //Channel = channel,
                    Order = order,
        ...
        public string ChargePay(ushort type, ushort channel, string order, decimal amount, string ip, string Ua, string subject, string body, string remark, string name, string no)
        {
            try
            {
                var request = new PayBank(channel, BankAccountType.DebitCard, name, no) // 指定银行编号，卡类型开户人和银行卡号
                    {
                        Order = order,
                        Currency = 1,
                        Amount = amount,
                ...
                var result = ChinaRailwayApp.GetInstance(AppId, PrivateKey, gateway_url).Pay.PayBank(request, out response); // 调用接口订单信息
                if (result.IsSuccess)
                {
        ...
        {
            var request = new QueryPay()
            {
                Order = orderId
            };
            QueryPayResult response;
            var app=ChinaRailwayApp.GetInstance(AppId, PrivateKey, gateway_url);
