...
namespace Common.Lottery.Gateway.Liangcai
{
    ...
    public class GatewayHandler_LC
    {
        ...
        public string QueryWinNumber(string gameCode, string issuseNumber)
        {
            ...
            var wParam = string.Format("LotID={0}_LotIssue={1}", LotID, LotIssue);
        ...
        public XmlDocument DoRequest(int action, string param)
        {
            ...
            var sign = string.Format("{0}{1}{2}{3}{4}", PartnerId, action, wMsgID, param, Key);
            var wSign = Encipherment.MD5(sign, Encoding.GetEncoding("GB2312")).ToLower();
            var request = string.Format("wAgent={0}&wAction={1}&wMsgID={2}&wSign={3}&wParam={4}", PartnerId, action,
                wMsgID, wSign, param);
            if (!string.IsNullOrEmpty(_xmlDir) && action == 101)
                SaveXML(param, action);
            var xml = PostManager.Post(ServiceUrl, request, Encoding.GetEncoding("gb2312"));
            if (string.IsNullOrEmpty(xml))
                throw new LogicException("返回结果不正确");
        ...
        public LC_ResposnseInfo DecodeResult(XmlDocument xmlDoc, string gameCode = "")
        {
            ...
            if (root == null)
                throw new LogicException("找不到该节点：ActionResult");
        ...
        public static void SaveXML(string content, int action)
        {
            ...
            if (doc != null)
            {
                ...
                if (node == null)
                {
                    var xe = doc.CreateElement("List");
                    doc.AppendChild(xe);
                }
                var strList = content.Split('^');
                foreach (var item in strList)
                {
                    var xe = doc.CreateElement("item");
                    ...
                    foreach (var order in orderList)
                    {
                        ...
                        var xe1 = doc.CreateElement(node_order);
        ...
        private static string GetLogFileName(int action)
        {
            ...
            return Path.Combine(_xmlDir, "LiangCai", "Ticket", DateTime.Today.ToString("yyyy-MM-dd"),
                DateTime.Now.Hour.ToString("D2"),
                string.Format("{0}-{1}-{2:D3}.xml", new object[] {action, _date, _index}));
        }
        ...
        private static void CreateXml(string path)
        {
            ...
            if (!File.Exists(path))
            {
                using (var fi = new FileStream(path, FileMode.Create))
                {
                }
                var doc = new XmlDocument();
