...
using System.Linq;
using System.Text;
using SystemManage.ApiGateway.Model.HelpModel;

namespace SystemManage.Api.Filter
{
    ...
    public class ReusltFilterAttribute : Attribute, IActionFilter
    {
        public string cxin = "★";
        public char cyuan = '●';
        public static ILogger<ReusltFilterAttribute> ilog=null;
        public ReusltFilterAttribute()
        {
            //、、 、、 Console.Write("");
            if(ilog==null)ilog = ServiceLocator.GetService<ILogger<ReusltFilterAttribute>>();
        }
        void IActionFilter.OnActionExecuted(ActionExecutedContext context)
        {
            ...
            if (result != null)
            {
                ...
                if (resp != null && resp.Code == ResponseCode.失败 && resp.Message.Contains(cxin))
                {
                    ...
                    ilog.LogError(new Exception(msg), "API或服务错误***");
                ...
                else if (resp.Code == ResponseCode.失败 && !resp.Message.Contains(cxin))
                {
                    ...
                    ilog.LogWarning(new Exception(msg), "API或服务错误***");
                }
            }
