...
//namespace app.lottery.admin.Controllers
//{
...
//            {
//                useable = bool.Parse(p.useable);
//            }
//            string BelongUserId = string.IsNullOrEmpty(p.belongUserId) ? string.Empty : p.belongUserId;
//            int PageIndex = string.IsNullOrEmpty(p.pageIndex) ? 0 : int.Parse(p.pageIndex);
//            int PageSize = string.IsNullOrEmpty(p.pageSize) ? 50 : int.Parse(p.pageSize);

//            var  List = _service.QueryCouponsList(Summary, useable, BelongUserId, PageIndex, PageSize, CurrentUser.UserToken);
//            return View();
//        }

//        public JsonResult BuildCoupons(LotteryServiceRequest entity)
//        {
//            try
//            {
//                var p = JsonHelper.Decode(entity.Param);
//                var summary = p.summary;
//                var money = decimal.Parse(p.money);
//                var count = int.Parse(p.count);
//                _service.BuildCoupons(summary, money, count, CurrentUser.UserToken);
