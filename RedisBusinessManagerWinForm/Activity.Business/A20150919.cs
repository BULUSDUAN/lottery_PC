...
namespace Activity.Business
{
    ...
    /// </summary>
    public class A20150919 : IAddBankCard, IResponseAuthentication_AfterTranCommit
    {
        /// <summary>
        /// 添加银行卡绑定 赠送1.5元彩金
        /// </summary>
        public void AddBankCard(string userId, string bankCardNumber, string bankCode, string bankName, string bankSubName, string cityName, string provinceName, string realName)
        {
            ...
            {
                //活动表中没有数据
                manager.AddA20150919_注册绑定送红包(new A20150919_注册绑定送红包
                {
                    UserId = userId,
        ...
        private void DoBindBankCardGiveMoney(string userId, A20150919_注册绑定送红包 record, A20150919Manager manager)
        {
        ...
        private void DoGiveMoney(string userId, A20150919_注册绑定送红包 record, A20150919Manager manager)
        {
    ...
    public class Yqid_Regist_GiveRedBag : IyqidRegister_AfterTranCommit
    {
        public void AfterRegisterTranCommit(string yqid)
        {
            ...
            if (giveMoney > 0M)
            {
                ...
                if (entity == null)
                {
                    managerA.AddBlog_UserSpreadGiveRedBag(new GameBiz.Domain.Entities.Blog_UserSpreadGiveRedBag
                    {
                        CreateTime = DateTime.Now,
                        giveRedBagMoney = 0,
                        GiveBonusMoney = 0,
                ...
                if (count > 0 && count % 10 == 0 && entity.redBagCount < count / 10 && entity.userGiveCount < count)
                {
                    ...
                    managerA.UpdateBlog_UserSpreadGiveRedBag(new GameBiz.Domain.Entities.Blog_UserSpreadGiveRedBag
                    {
                        UpdateTime = DateTime.Now,
    ...
    public class A20150919_Regist_GiveRedBag : IRegister_AfterTranCommit
    {
        public void AfterRegisterTranCommit(string regType, string userId)
        {
            ...
            if (giveMoney > 0M)
            {
                ...
                manager.AddA20150919_注册送红包(new A20150919_注册送红包
                {
                    CreateTime = DateTime.Now,
    ...
    public class A20150919_BindRealName_Mobile_Login : IUser_AfterLogin
    {
        public void User_AfterLogin(string userId, string loginFrom, string loginIp, DateTime loginTime)
        {
            ...
            if (giveMoney > 0)
            {
                ...
                manager.AddA20150919_已绑定身份和手机的用户登录送红包(new A20150919_已绑定身份和手机的用户登录送红包
                {
                    CreateTime = DateTime.Now,
                    UserId = userId,
                    LoginDate = date,
    ...
    /// 充值赠送红包
    /// </summary>
    public class A20150919_FillMoneyGive : ICompleteFillMoney_AfterTranCommit
    {
            ...
            if (!string.IsNullOrEmpty(PayRedBagConfig))
            {
                ...
                if (dict.ContainsKey(key))
                {
                    ...
                    if (money > 0)
                    {
                        ...
                        //添加赠送记录
                        manager.AddA20150919_充值送红包记录(new A20150919_充值送红包记录
                        {
                            CreateTime = DateTime.Now,
                            UserId = userId,
                            OrderId = orderId,
        ...
        {
            var manager = new A20150919Manager();
            manager.AddA20150919_充值送红包配置(new A20150919_充值送红包配置
            {
                CreateTime = DateTime.Now,
                FillMoney = fillMoney,
                GiveMoney = giveMoney,
        ...
        public void FirstRechargeGiveRedBag(string userId, decimal totalMoney)
        {
            using (var biz = new GameBizBusinessManagement())
            {
                ...
                {
                    var entityShareSpread = new BlogManager().QueryBlog_UserShareSpread(userId);
                    var satisfyFillMoney = decimal.Parse(Activity.Business.ActivityCache.QueryActivityConfig("ActivityConfig.SatisfyRechargeGiveRedBagTofxid").ConfigValue);
                    if (entityShareSpread != null && !entityShareSpread.isGiveRechargeRedBag && totalMoney >= satisfyFillMoney)
                    {
                        //购彩了 没有给分享者送活动红包 就执行送红包 只送一次
                        var giveFillMoney = decimal.Parse(Activity.Business.ActivityCache.QueryActivityConfig("ActivityConfig.FirstRechargeGiveRedBagTofxid").ConfigValue);
                        if (giveFillMoney > 0)
                        {
                            BusinessHelper.Payin_To_Balance(AccountType.RedBag, BusinessHelper.FundCategory_Activity, entityShareSpread.AgentId, Guid.NewGuid().ToString("N"), giveFillMoney
    ...
    /// 彩种加奖相关
    /// </summary>
    public class A20150919_AddBonusMoney : IOrderPrize_AfterTranCommit
    {
        ...
        private A20150919_加奖配置 FindConfig(List<A20150919_加奖配置> configList, string gameCode, string gameType, string playType)
        {
        ...
        private A20150919_加奖配置 FindConfig(List<A20150919_加奖配置> configList, GameBiz.Domain.Entities.Sports_Ticket ticket)
        {
            foreach (var config in configList.OrderBy(p => p.OrderIndex).ToArray())
            {
        ...
        private decimal ComputeAddMoney(A20150919_加奖配置 rule, decimal bonusMoney)
        {
        ...
        {
            var manager = new A20150919Manager();
            manager.AddA20150919_加奖配置(new A20150919_加奖配置
            {
    ...
    public class A20150919_RedBagUseConfig
    {
        ...
        public void AddRedBagUseConfig(string gameCode, decimal percent)
        {
            ...
            manager.AddA20150919_红包使用配置(new A20150919_红包使用配置
            {
                CreateTime = DateTime.Now,
                GameCode = gameCode,
                UsePercent = percent
    ...
    public class A20150919_UserGameCodeNotAddMoney
    {
        ...
        public void AddUserGameCodeNotAddMoney(string userId, string gameCode, string gameType, string playType)
        {
            ...
            activityManager.AddA20150919_列表用户不加奖(new A20150919_列表用户不加奖
            {
                CreateTime = DateTime.Now,
                GameCode = gameCode,
                UserId = userId,
