...
using External.Domain.Managers.Activity;
using System.IO;
...
namespace External.Business
{
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            if (old == null)
            {
                ...
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    TaskCategory = TaskCategory.FistBuyLottery,
                    TaskName = "首次购买彩票",
        ...
        public void CompleteFillMoney_AfterTranCommit(string orderId, FillMoneyStatus status, FillMoneyAgentType agentType, decimal fillMoney, string userId, int vipLevel)
        {
            ...
            if (count == 1)
            {
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    OrderId = orderId,
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.FistTopUp,
                    TaskName = "首次充值",
                    CreateTime = DateTime.Now,
                });
                var user = balanceManager.QueryUserRegister(userId);
        ...
        public void AddBalancePassword(string userId, string oldPassword, bool isSetPwd, string newPassword)
        {
            ...
            if (old == null)
            {
                ...
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    OrderId = order,
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.SetBalancePassword,
                    TaskName = "设置资金密码",
                    CreateTime = DateTime.Now,
                });
                gv.AddTaskList(new TaskList
        ...
        public void AddBankCard(string userId, string bankCardNumber, string bankCode, string bankName, string bankSubName, string cityName, string provinceName, string realName)
        {
            ...
            if (old == null)
            {
                ...
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    OrderId = order,
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.BankCar,
                    TaskName = "绑定银行卡",
                    CreateTime = DateTime.Now,
                });
                gv.AddTaskList(new TaskList
    ...
    public class FistJingcaiP2_1Business : IComplateTicket
    {
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            if (old == null && SchemeType.GeneralBetting == order.SchemeType && (order.GameCode == "JCLQ" || order.GameCode == "JCZQ") && order.GameType != "HH" && order.PlayType == "2_1")
            {
                //查询vip等级
                var balanceManager = new UserBalanceManager();
                var user = balanceManager.QueryUserRegister(userId);
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    OrderId = schemeId,
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.FistJingcaiP2_1,
                    TaskName = "购买竞彩2串1",
                    CreateTime = DateTime.Now,
                });
                if (user.VipLevel >= 3)
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            if (old == null && SchemeType.GeneralBetting == order.SchemeType && (order.GameCode == "JCLQ" || order.GameCode == "JCZQ") && order.GameType == "HH" && order.PlayType == "2_1")
            {
                ...
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    OrderId = schemeId,
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.FistHHP2_1,
                    TaskName = "购买竞彩混投2串1",
                    CreateTime = DateTime.Now,
                });
                if (user.VipLevel >= 3)
    ...
    public class JingcaiFistWinBusiness : IOrderPrize_AfterTranCommit
    {
        ...
        public void OrderPrize_AfterTranCommit(string userId, string schemeId, string gameCode, string gameType, string issuseNumber, decimal orderMoney, bool isBonus, decimal preTaxBonusMoney, decimal afterTaxBonusMoney, bool isVirtualOrder, DateTime prizeTime)
        {
            ...
            if (old == null && SchemeType.GeneralBetting == order.SchemeType && (order.GameCode == "JCLQ" || order.GameCode == "JCZQ"))
            {
                //查询vip等级
                var balanceManager = new UserBalanceManager();
                var user = balanceManager.QueryUserRegister(userId);
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    OrderId = schemeId,
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.JingcaiFistWin,
                    TaskName = "竞彩首次中奖",
                    CreateTime = DateTime.Now,
                });
                if (user.VipLevel >= 3)
    ...
    public class FistZhuihaoBuyBusiness : IComplateTicket
    {
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            if (order == null) return;
            if (order.IsVirtualOrder) return;
            if (SchemeType.ChaseBetting != order.SchemeType)
                return;
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.FistZhuihaoBuy,
                TaskName = "首次追号投注",
                CreateTime = DateTime.Now,
            });
            if (user.VipLevel >= 3)
    ...
    public class FistHeMaiBusiness : IComplateTicket
    {
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            if (order == null) return;
            if (order.IsVirtualOrder) return;
            if (order.SchemeType != SchemeType.TogetherBetting) return;
        ...
        public void Attention_AfterTranCommit(string activeUserId, string passiveUserId)
        {
            ...
            if (old == null)
            {
                ...
                gv.AddUserTaskRecord(new UserTaskRecord
                {
                    ...
                    CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                    TaskCategory = TaskCategory.FistFocusOnFriend,
                    TaskName = "关注彩友",
                    CreateTime = DateTime.Now,
                });
                if (user.VipLevel >= 3)
    ...
    public class LoginHistoryBusiness : IUser_AfterLogin
    {
        ...
        public void User_AfterLogin(string userId, string loginFrom, string loginIp, DateTime loginTime)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                TaskCategory = TaskCategory.EveryDayLogin,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                CreateTime = DateTime.Now,
                TaskName = "每日登录",
                UserId = userId,
        ...
        public int GetUserTaskRecordSort(List<UserTaskRecord> taskList)
        {
            var giveCount = 1;
    ...
    public class CumulativeTenYuanBusiness : IComplateTicket
    {
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.FistConsumptionTenYuan,
                TaskName = "累计购彩满10元",
                CreateTime = DateTime.Now,
            });
            if (user.VipLevel >= 2)
    ...
    public class EverDayBuyLotteryBusiness : IComplateTicket
    {
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = today,
                TaskCategory = TaskCategory.EverDayBuyLottery,
                TaskName = "每日购彩≥50元",
                CreateTime = DateTime.Now,
            });
    ...
    public class JingcaiP2_1Totle5Business : IComplateTicket
    {
        ...
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = today,
                TaskCategory = TaskCategory.JingcaiP2_1Totle5,
                TaskName = "购买竞彩2串1满5次",
                CreateTime = DateTime.Now,
            });
    ...
    public class Win100CountBusiness : IOrderPrize_AfterTranCommit
    {
            ...
            bool isBonus, decimal preTaxBonusMoney, decimal afterTaxBonusMoney, bool isVirtualOrder, DateTime prizeTime)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.Win100Count,
                TaskName = "中奖100次",
                CreateTime = DateTime.Now,
            });
    ...
    public class JCWin100CountBusiness : IOrderPrize_AfterTranCommit
    {
            ...
            bool isBonus, decimal preTaxBonusMoney, decimal afterTaxBonusMoney, bool isVirtualOrder, DateTime prizeTime)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.JCWin100Count,
                TaskName = "竞彩中奖100次",
                CreateTime = DateTime.Now,
            });
    ...
    public class Win1000YuanBusiness : IOrderPrize_AfterTranCommit
    {
            ...
            bool isBonus, decimal preTaxBonusMoney, decimal afterTaxBonusMoney, bool isVirtualOrder, DateTime prizeTime)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.Win1000Yuan,
    ...
    public class BonusOptimize : IBonusOptimize
    {
        public void AddBonusOptimize(string userId, string schemeId)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.BonusOptimize,
                TaskName = "首次使用奖金优化",
                CreateTime = DateTime.Now,
            });
            if (user.VipLevel >= 4)
    ...
    public class BonusOptimizeTotal5 : IBonusOptimize
    {
        public void AddBonusOptimize(string userId, string schemeId)
        {
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.BonusBuyLotteryTotle5,
                TaskName = "奖金优化投注满5次",
                CreateTime = DateTime.Now,
            });
    ...
    public class SpecialWinReward : IOrderPrize_AfterTranCommit
    {
            ...
            bool isBonus, decimal preTaxBonusMoney, decimal afterTaxBonusMoney, bool isVirtualOrder, DateTime prizeTime)
        {
            ...
            if (order == null)
                return;
            if (order.SchemeBettingCategory != SchemeBettingCategory.YouHua)
                return;
            ...
            gv.AddUserTaskRecord(new UserTaskRecord
            {
                ...
                OrderId = schemeId,
                CurrentTime = DateTime.Now.ToString("yyyyMMdd"),
                TaskCategory = TaskCategory.SpecialWinReward,
                TaskName = "特别中奖奖励",
                CreateTime = DateTime.Now,
            });
            if (user.VipLevel >= 4)
    ...
    /// Vip等级增加
    /// </summary>
    //public class VipLevel : AddVipLevel
    //{
    //    public void AddVipLevel(string userId, int vipLevel)
    //    {
    //        var gv = new TaskListManager();
    //        var gvList = gv.QueryTaskList(userId, vipLevel);
    //        foreach (var item in gvList)
    //        {
    //            if (!item.IsGive)
    //            {
    //                BusinessHelper.Payin_UserGrowth(item.TaskName, item.OrderId, userId, (int)item.ValueGrowth, item.Content, isExecPlugin: false);
    //                item.IsGive = true;
    //                gv.UpdateTaskList(item);
    //            }
    //        }
    //    }
    ...
    //            {
    //                case "AddVipLevel":
    //                    AddVipLevel((string)paraList[0], (int)paraList[1]);
    //                    break;
    //                default:
    //                    throw new ArgumentOutOfRangeException("不支持的插件类型 - " + type);
    //            }
    ...
    //        {
    //            var writer = Common.Log.LogWriterGetter.GetLogWriter();
    //            writer.Write("EXEC_AddVipLevel_Error_", type, ex);
    //        }
    ...
    /// 活动列表查询
    /// </summary>
    public class ActivListBusiness
    {
        /// <summary>
        ///查询活动列表查询
        /// </summary>
        ...
        {
            var result = new ActivityListInfoCollection();
            var totalCount = 0;
            result.List.AddRange(new ActivityListManager().QueryActivInfoList(pageIndex, pageSize, out totalCount));
            result.TotalCount = totalCount;
            return result;
        }
        ...
        {
            return new ActivityListManager().QueryActivInfo(activeName);
        }
