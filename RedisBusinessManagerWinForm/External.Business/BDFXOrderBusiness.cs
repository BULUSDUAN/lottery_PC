...
using External.Business.Domain.Managers.BDFX;
using GameBiz.Business;

namespace External.Business
{
    public class BDFXOrderBusiness : IComplateTicket, IOrderPrize_AfterTranCommit
    {
        /// <summary>
        /// 宝单分享
        /// </summary>
        public void ComplateTicket(string userId, string schemeId, decimal totalMoney, decimal totalErrorMoney)
        {
            try
            {
                ...
                if (orderRunning == null)
                    return;
                if (orderRunning.SchemeType == SchemeType.SingleTreasure)
                {
                    ...
                    if (BDFXOrderEntity == null || string.IsNullOrEmpty(BDFXOrderEntity.UserId))//晒单
                    {
                        ...
                        BDFXOrderEntity = new TotalSingleTreasure();
                    ...
                    {
                        SingleTreasureAttentionSummary entity = new SingleTreasureAttentionSummary();
                ...
                else if (orderRunning.SchemeType == SchemeType.SingleCopy)//抄单
                {
        ...
        public void OrderPrize_AfterTranCommit(string userId, string schemeId, string gameCode, string gameType, string issuseNumber, decimal orderMoney, bool isBonus, decimal preTaxBonusMoney, decimal afterTaxBonusMoney, bool isVirtualOrder, DateTime prizeTime)
        {
            ...
            if (orderComplate.SchemeType == SchemeType.SingleTreasure)//宝单
            {
            ...
            else if (orderComplate.SchemeType == SchemeType.SingleCopy && afterTaxBonusMoney > 0)//抄单
            {
        ...
        {
            using (var manager = new External.Business.Domain.Managers.RestrictionsBetMoney.RestrictionsBetMoneyManager())
            {
