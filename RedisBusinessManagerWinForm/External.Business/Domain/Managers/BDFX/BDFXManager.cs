...
namespace External.Business.Domain.Managers.BDFX
{
    public class BDFXManager : GameBizEntityManagement
    {
        ...
        public void AddSingleTreasureAttentionSummary(SingleTreasureAttentionSummary entity)
        {
            this.Add<SingleTreasureAttentionSummary>(entity);
        }
        public void UpdateSingleTreasureAttentionSummary(SingleTreasureAttentionSummary entity)
        {
            this.Update<SingleTreasureAttentionSummary>(entity);
        }
        public SingleTreasureAttentionSummary QuerySingleTreasureAttentionSummaryByUserId(string userId)
        {
            Session.Clear();
            return Session.Query<SingleTreasureAttentionSummary>().FirstOrDefault(s => s.UserId == userId);
        }
        ...
        public SingleTreasureAttention QuerySingleTreasureAttentionByUserId(string beConcernedUserId, string concernedUserId)
        {
            Session.Clear();
            return Session.Query<SingleTreasureAttention>().FirstOrDefault(s => s.BeConcernedUserId == beConcernedUserId && s.ConcernedUserId == concernedUserId);
        }

        public List<SingleTreasureAttentionInfo> QueryBDFXAllAttentionIList()
        {
            Session.Clear();
            var query = from a in Session.Query<SingleTreasureAttention>()
                        select new SingleTreasureAttentionInfo
                        {
                            Id = a.Id,
            ...
            return new List<SingleTreasureAttentionInfo>();
        }
        public List<string> QueryBeConcernedUserIdList(string concernedUserId)
        {
            Session.Clear();
            return (from o in Session.Query<SingleTreasureAttention>().Where(s => s.ConcernedUserId == concernedUserId) select o.BeConcernedUserId).ToList();
        }
        public List<AnteCodeInfo> QueryAnteCodeList(string[] arrSchemeId)
        {
            Session.Clear();
            var query = from a in Session.Query<Sports_AnteCode>()
        ...
        {
            Session.Clear();
            var orderRunning = Session.Query<Sports_Order_Running>().FirstOrDefault(s => s.SchemeId == schemeId);
            if (orderRunning != null && !string.IsNullOrEmpty(orderRunning.SchemeId))
            {
                var query = from t in Session.Query<TotalSingleTreasure>()
                            join o in Session.Query<Sports_Order_Running>() on t.SchemeId equals o.SchemeId
            ...
            {
                var query = from t in Session.Query<TotalSingleTreasure>()
                            join o in Session.Query<Sports_Order_Complate>() on t.SchemeId equals o.SchemeId
                            join u in Session.Query<UserRegister>() on t.UserId equals u.UserId
                            where t.SchemeId == schemeId
                            select new BDFXOrderDetailInfo
                            {
