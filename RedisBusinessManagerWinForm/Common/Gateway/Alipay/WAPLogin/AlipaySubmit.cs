...
namespace Common.Gateway.Alipay.WAPLogin
{
    /// <summary>
    /// 类名：Submit
    /// 功能：支付宝各接口请求提交类
    ...
    public class LoginSubmit
    {
        ...
        //支付宝网关地址（旧）
        private static string GATEWAY_OLD = "https://www.alipay.com/cooperate/gateway.do?";
        //支付宝WAP快捷登录用户信息查询
        private static string GATEWAY_USERINFO = "http://mapi.sit.alipay.net/gateway.do?";
        #endregion

        /// <summary>
        /// 初始化请求提交类
        /// </summary>
        /// <param name="partner">商户ID，使用伙伴ID</param>
        /// <param name="key">安全校验码，与partner是一组的</param>
        public LoginSubmit(string partner, string key)
        {
        ...
        /// 构造跳转URL
        /// </summary>
        /// <param name="return_url">回调页面</param>
        /// <param name="return_url_failed">登录失败回调页面</param>
        /// <param name="request_id">商户方传入的订单流号，不可空</param>
        /// <returns></returns>
        public string BuildRequestString(string return_url, string return_url_failed, string request_id)
        {
            //临时请求参数数组
            SortedDictionary<string, string> sParaTemp = new SortedDictionary<string, string>();
            Config.Return_url = return_url;
            //构造请求参数数组
            sParaTemp.Add("service", "wap.user.common.login");
            sParaTemp.Add("login_service", "user_auth");
            sParaTemp.Add("partner", Config.Partner.Trim());
            sParaTemp.Add("input_charset", _input_charset);
            sParaTemp.Add("return_url", Config.Return_url.Trim());
            sParaTemp.Add("return_url_failed", Config.Return_url.Trim());
            sParaTemp.Add("request_id", request_id);
            sParaTemp.Add("anti_phishing_key", "");///////////     防钓鱼参数不设置    ////////
            sParaTemp.Add("exter_invoke_ip", "");
            return GATEWAY_NEW + BuildRequestParaToString(sParaTemp);
        }
        ...
        public string wap_login_userinfo_query(SortedDictionary<string, string> sParaTemp)
        {
            //增加基本配置
            sParaTemp.Add("service", "mobile.common.login.userInfo.query");
            sParaTemp.Add("partner", Config.Partner.Trim());
