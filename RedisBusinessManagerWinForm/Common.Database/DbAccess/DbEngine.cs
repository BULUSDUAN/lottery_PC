...
namespace Common.Database.DbAccess
{
    internal class DbEngine
    {
        ...
        private void OpenDB(DbConnection dbConn, string connString)
        {
            ...
            {
                string str = HiddenConnStringPassword(dbConn.ConnectionString);
                throw new DbAccessException("打开数据库连接失败，" + "ConnectionString: " + str, ex);
            }
        }
        private string HiddenConnStringPassword(string connString)
        {
            DbConnectionStringBuilder buider = new DbConnectionStringBuilder();
            buider.ConnectionString = connString;
            if (buider.ContainsKey("Password"))
            {
                buider["Password"] = "******";
            }
            else if (buider.ContainsKey("Pwd"))
            {
                buider["Pwd"] = "******";
            }
            return buider.ConnectionString;
        }
