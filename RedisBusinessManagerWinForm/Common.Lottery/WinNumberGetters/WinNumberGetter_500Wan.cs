...
namespace Common.Lottery.WinNumberGetters
{
    ...
    {
        private static Dictionary<string, string> UrlMapping = null;
        static WinNumberGetter_500Wan()
        {
            ...
            UrlMapping.Add("GD11X5", "http://kaijiang.500.com/static/info/kaijiang/xml/gdsyxw/{0:yyyyMMdd}.xml");//广东11选5 
        ...
        {
            string xmlUrl = GetUrl(gameName, lastIssuseCount);
        ...
        {
            Dictionary<string, string> result = new Dictionary<string, string>();
            XmlNode root = xmlDoc.GetElementsByTagName("xml")[0];
            int start = root.ChildNodes.Count - 1;

            for (; start >= 0; start--)
            {
                XmlElement ele = root.ChildNodes[start] as XmlElement;
                string issuse = ele.Attributes["expect"].Value;
                string winCode = ele.Attributes["opencode"].Value;
                switch (gameCode)
                {
                    case "SDQYH":
        ...
        private Dictionary<string, string> GetWinNumber1(XmlDocument xmlDoc, int lastIssuseCount, string gameCode)
        {
            ...
            {
                string winCode = item.Attributes["opencode"].Value;
                string issuse = item.Attributes["expect"].Value;
                switch (gameCode)
                {
                    case "SSQ":
                    case "QXC":
