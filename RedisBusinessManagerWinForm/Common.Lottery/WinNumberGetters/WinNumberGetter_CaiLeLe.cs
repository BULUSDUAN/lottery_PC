...
namespace Common.Lottery.WinNumberGetters
{
    internal class WinNumberGetter_CaiLeLe : WinNumberGetter
    {
        ...
        //todo:广西快乐十分
        private const string urlGXKL10F = @"http://kjh.cailele.com/history_gxklsf.aspx";
        //山东群英会
        ...
        //todo:上海时时乐
        private const string urlSHSSL = @"http://kjh.cailele.com/history_shssl.aspx?menu=mcl_4";
        //江苏快3
        ...
        private string FomartWinNumber(string gameCode, string winNumber)
        {
            switch (gameCode)
            {
                ...
                case "QXC":
                    break;
                case "HC1":
                    var hc1Array = winNumber.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                    return hc1Array[0];
                case "SSQ":
                    var ssqArray = winNumber.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                    if (ssqArray.Length != 7)
                        throw new Exception("双色球应该是由 , 分隔的7位数");
                    var ssqQian = string.Join(",", ssqArray.Take(6).ToArray());
                    var ssqHou = ssqArray[6];
                    return string.Join("|", ssqQian, ssqHou);
                case "DF6J1":
                    var df6j1Array = winNumber.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                    if (df6j1Array.Length != 7)
                        throw new Exception("东方六加一应该是由 , 分隔的7位数");
                    var df6j1Qian = string.Join(",", df6j1Array.Take(6).ToArray());
                    var df6j1Hou = df6j1Array[6];
                    return string.Join("|", df6j1Qian, df6j1Hou);
                case "QLC":
                    var qlcArray = winNumber.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                    if (qlcArray.Length != 8)
                        throw new Exception("七乐彩应该是由 , 分隔的8位数");
                    var qlcQian = string.Join(",", qlcArray.Take(7).ToArray());
                    var qlcHou = qlcArray[7];
                    return string.Join("|", qlcQian, qlcHou);
                case "DLT":
                    var dltArray = winNumber.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
