...
namespace KaSon.FrameWork.ORM.Helper.AnalyzerFactory.Model
{
    ...
    /// 玩法分析 - 前N直选
    /// </summary>
    internal class GameTypeAnalyzer_前N直选 : IAntecodeAnalyzable
    {
        public GameTypeAnalyzer_前N直选()
        {
            Spliter = ',';
            TotalNumber = 5;
        }
        ...
        public IList<int> CaculateBonus(string antecode, string winNumber)
        {
            ...
            {
                string[] one = AnteCodeNumbers[i].Split(' ');
                if (!one.Contains(checker.WinNumbers[i]))
                {
                    isWin = false;
                    break;
                }
