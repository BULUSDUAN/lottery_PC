...
{
    public class GameAnalyzer_快乐扑克3 : IOrderAnalyzable, IWinNumberAnalyzable
    {
        public GameAnalyzer_快乐扑克3(string gameCode)
        {
            GameCode = gameCode;
            Spliter = ',';
            TotalNumber = 3;
        ...
        public bool CheckOneAntecodeNumber(string antecodeNumber, out string errMsg, string type = null)
        {
            ...
            {
                if (number < 101 || number > 413)
                {
                    errMsg = "号码必须是 101 - 413 之间的数字";
                    return false;
                }
            }
            else
