...
{
    [ModuleName("NumCraw")]
    public class NumCrawService : INumCrawService
    {
        ...
        public Task<string> NumLettory_HK6Issuse(string gameName = "HK6")
        {
                        ...
                        if (p2 == null)
                        {
                            ...
                            System.Timers.Timer timer = ServiceHelper.ExcuteByTimer(initTimeData, () =>
                            {
                                ...
                                try
                                {
                                    ...
                                    Console.WriteLine("1结算timer 启动");
                                    _log.LogTrace("OpenWinHK6 启动" + DateTime.Now.ToShortDateString());
                                    Service_AutoCollectIssuse auto = new Service_AutoCollectIssuse(rep.MDB, gameName);
            ...
            return Task.FromResult("成功开启服务" + gameName);
        }
        public CommonActionResult NumLettory_Sum(string userId = "",string IssueNo="",string winNum="")
        {
            CrawORMService datahelper = new CrawORMService(rep.MDB);
            blast_data bdata = new blast_data() {
                 kjdata=winNum,
                 issueNo=int.Parse( IssueNo)

            };
           var result= datahelper.HK6OpenwinNumByIssus(userId, bdata);
           // #endregion

            return result;
        }
        public CommonActionResult NumLettory_BJPKSum(string userId = "", string IssueNo = "", string winNum = "")
        {
            CrawORMService datahelper = new CrawORMService(rep.MDB);
            blast_data bdata = new blast_data()
            {
        ...
        public Task<string> NumLettory_BJPK(string gameName = "BJPK") {
            ...
            return Task.FromResult("BJPK采集开奖号并结算服务启动" + gameName);
        }


        public bool UpdateErrorIssue() {
            CrawORMService datahelper = new CrawORMService(rep.MDB);
            datahelper.UpdateErrorIssue();
            return true;
        }
        //WinNumber
    }
