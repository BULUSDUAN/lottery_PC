  ...
  public  class DbProvider: IDbProvider
    {
      ...
      private bool _isTran;
        ...
        public IDbProvider Push(string dbkey, bool isLLink = false)
        {
            IDbProvider ipProvider = null;
            if (!GlobalCache.DbProviderPool.Keys.Contains(dbkey))
            {
                GlobalCache.DbProviderPool.TryGetValue(dbkey,out ipProvider);
            }
            else {
             
                ipProvider =  this.Init(dbkey, isLLink);
                GlobalCache.DbProviderPool.Add(dbkey, ipProvider);
            }
            return ipProvider;
        }
      public IDbProvider Init(string dbkey, bool isLLink = false)
      {
        ...
        /// 设置保持连接状态
        /// </summary>
        public bool SetLink { set { this._isLLink = value;this._factory.IsLLink = this._isLLink; } }
      ...
      public KaSon.FrameWork.Services.ORM.IDbHelper GetDbHelper()
      {
          if (!KaSon.FrameWork.ORM.GlobalCache.DbHelperPool.Keys.Contains(this._dbKey))
          {
                ...
                if (!string.IsNullOrEmpty(this._dbKey)  && this._factory != null)
                {
                    Console.WriteLine("dh.......");
                    IDbHelper helper = this._factory.CreateDbHelper();
                    GlobalCache.DbHelperPool.Add(_dbKey, helper);
                }
                else {
